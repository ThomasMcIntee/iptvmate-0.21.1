---
title: Understanding the Electronic Program Guide
description: Learn how iptvmate uses XMLTV data to display current and upcoming programs alongside your live TV channels.
pubDate: 2026-02-14
author: 4gray
heroImage: /iptvmate/blog/epg.png
tags:
- tutorial
- epg
draft: false
---

import Alert from '../../components/blog/Alert.astro';
import PostButton from '../../components/blog/PostButton.astro';

One of the most useful features in iptvmate is the built-in Electronic Program Guide (EPG). Here is how it works and how to set it up.

## What is EPG?

The Electronic Program Guide displays a schedule of current and upcoming TV programs for your channels. iptvmate supports EPG data in the **XMLTV format**, which is the standard used by most IPTV providers.

<Alert type="info" title="Format tip">
  Most providers expose EPG as `.xml` or compressed `.xml.gz` files.
</Alert>

## Setting up EPG

To enable EPG in iptvmate:

1. **Open your playlist** and navigate to the Settings tab
2. **Add an EPG URL** — your IPTV provider should supply a URL ending in `.xml` or `.xml.gz`
3. **Refresh the EPG data** — iptvmate will download and parse the program information in the background

The EPG data is parsed in a background worker thread, so it won't slow down the main application even with large files.

## EPG in action

Once configured, you will see:

- **Current program** displayed next to each channel in the channel list
- **Progress bar** showing how far through the current program you are
- **Program details** in the right sidebar when you select a channel
- **Upcoming schedule** with times and descriptions for future programs

## Channel matching

iptvmate matches EPG data to channels using the `tvg-id` attribute from your M3U playlist. Make sure your playlist includes these IDs for the best EPG experience.

## Tips

- EPG files can be large — iptvmate caches parsed data in the database (Electron)
- You can add multiple EPG sources per playlist
- The EPG automatically refreshes periodically to keep the schedule up to date

<PostButton
  href="https://github.com/4gray/iptvmate"
  label="See supported formats on GitHub"
/>
